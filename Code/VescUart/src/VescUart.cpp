...
int VescUart::receiveUartMessage(uint8_t *payloadReceived)
{
	...
	while (millis() < timeout && messageRead == false)
	{
		while (serialPort->available())
		{
			...
			{
				debugPort->println("Buffer overflow");
				break;
			}
...
int VescUart::packSendPayload(uint8_t *payload, int lenPay)
{
	...
	if (debugPort != NULL)
	{
		debugPort->println("Package sent");
	}
...
bool VescUart::processReadPacket(uint8_t *message)
{
	...
	case COMM_GET_VALUES:													  // Structure defined here: https://github.com/vedderb/bldc/blob/43c3bbaf91f5052a35b75c2ff17b5fe99fad94d1/commands.c#L164
		...
		data.Id = buffer_get_float32(message, 100.0, &index);				  // 4 bytes - mc_interface_read_reset_avg_id()
		data.Iq = buffer_get_float32(message, 100.0, &index);				  // 4 bytes - mc_interface_read_reset_avg_iq()
		...
		data.ntcTemp1 = buffer_get_float16(message, 10.0, &index);			  // 2 bytes - NTC_TEMP_MOS1()
		data.ntcTemp2 = buffer_get_float16(message, 10.0, &index);			  // 2 bytes - NTC_TEMP_MOS2()
		data.ntcTemp3 = buffer_get_float16(message, 10.0, &index);			  // 2 bytes - NTC_TEMP_MOS3()
		data.Vd = buffer_get_float16(message, 1000.0, &index);				  // 2 bytes - mc_interface_read_reset_avg_vd()
		data.Vq = buffer_get_float16(message, 1000.0, &index);				  // 2 bytes - mc_interface_read_reset_avg_vq()
		// 8 bit consist of 2 set of information, 2 bit at LSB				  // 1 byte - 1 bit timedOut, 1 bit timeoutSwitchActive
		data.timedOut = (message[index++] & 0x01) == 0x01;
		data.timeoutSwitchActive = (message[index] & 0x02) == 0x02;
		return true;

	case COMM_GET_VALUES_SELECTIVE:
		index += 4;															  // Skip 4 bytes, they are the mask
		data.avgMotorCurrent = buffer_get_float32(message, 100.0, &index);	  // 4 bytes - mc_interface_read_reset_avg_motor_current()
		data.avgInputCurrent = buffer_get_float32(message, 100.0, &index);	  // 4 bytes - mc_interface_read_reset_avg_input_current()
		data.dutyCycleNow = buffer_get_float16(message, 1000.0, &index);	  // 2 bytes - mc_interface_get_duty_cycle_now()
		data.rpm = buffer_get_float32(message, 1.0, &index);				  // 4 bytes - mc_interface_get_rpm()
...
bool VescUart::getVescValuesSelective(uint32_t mask)
{
	if (debugPort != NULL)
	{
		// print mask in the format of 32 bits int 0x
		debugPort->println("Mask: " + String(mask, HEX));
	}

	return getVescValuesSelective(mask, 0);
}

bool VescUart::getVescValuesSelective(uint32_t mask, uint8_t canId)
{

	if (debugPort != NULL)
	{
		debugPort->println("Command: COMM_GET_VALUES_SELECTIVE " + String(canId));
	}
...
void VescUart::setDuty(float duty, uint8_t canId)
{
	...
	if (debugPort != NULL)
	{
		debugPort->print("Duty: ");
		debugPort->println(duty);
	}
}
...
void VescUart::printVescValues()
{
	if (debugPort != NULL)
	{
		...
		debugPort->print("pidPos: ");
		debugPort->println(data.pidPos);
		debugPort->print("id: ");
		debugPort->println(data.id);
		debugPort->print("ntcTemp1: ");
		debugPort->println(data.ntcTemp1);
		debugPort->print("ntcTemp2: ");
		debugPort->println(data.ntcTemp2);
		debugPort->print("ntcTemp3: ");
		debugPort->println(data.ntcTemp3);
		debugPort->print("resetVd: ");
		debugPort->println(data.Vd);
		debugPort->print("resetVq: ");
		debugPort->println(data.Vq);
		debugPort->print("timedOut: ");
		debugPort->println(data.timedOut);
		debugPort->print("timeoutSwitchActive: ");
		debugPort->println(data.timeoutSwitchActive);
	}
}
